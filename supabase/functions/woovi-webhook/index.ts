...
Deno.serve(async (req) => {
  // Health-checks and browser probes.
  ...
  try {
    ...
    // Empty/non-JSON probes must be acknowledged.
    if (!rawBody.trim()) return new Response(null, { status: 200 });
    ...
    // Woovi validates a new webhook with a POST body containing only
    // data_criacao + event. Their docs require HTTP 200 with an empty body.
    const isWooviValidationProbe =
      typeof payload?.event === 'string' &&
      typeof payload?.data_criacao === 'string' &&
      !payload?.charge &&
      !payload?.data?.charge &&
      !payload?.correlationID;

    if (isWooviValidationProbe) {
      return new Response(null, { status: 200 });
    }
    ...
    // Unknown/non-charge payloads are harmless and should not trigger retries.
    if (!correlationID) return new Response(null, { status: 200 });
