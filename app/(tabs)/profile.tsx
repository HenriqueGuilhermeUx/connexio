...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    if (!supabase || !member) { setIsAdmin(false); return; }
    void supabase.rpc('is_connexio_admin').then(({ data, error }) => { if (!error) setIsAdmin(Boolean(data)); });
  }, [member?.id]);
  ...
  return <Screen contentStyle={styles.content}>
    ...
    {isAdmin?<View style={styles.section}><Text style={styles.sectionTitle}>Admin Connexio</Text><Button label="Abrir painel administrativo" variant="secondary" onPress={()=>router.push('/admin')}/><Text style={styles.adminNote}>Acesso administrativo separado das permissões de cada Loja.</Text></View>:null}
