...
import { createLodgeCharge } from '@/lib/paymentRepository';
...
export default function ManagerVoiceScreen(){
  ...
  const execute=async()=>{
    ...
    try{
      ...
      if(preview.kind==='CHARGE'){
        await createLodgeCharge({lodgeId:lodge.id,memberId:preview.memberId,memberName:preview.memberName,memberEmail:preview.memberEmail,memberPhone:preview.memberPhone,description:preview.description,amount:preview.amount,dueDate:preview.dueDate});
        setFeedback({type:'success',text:`Cobrança criada para ${preview.memberName}. Ela usará a chave Pix/QR da própria Loja em Cobranças.`});
      }
  ...
  return <Screen contentStyle={styles.content}>
    ...
    <View style={styles.commandCard}><Pressable onPress={listen} style={[styles.listenButton,listening&&styles.listening]}><Feather name={listening?'radio':'mic'} size={25} color={listening?colors.background:colors.gold}/><Text style={[styles.listenText,listening&&styles.listenTextOn]}>{listening?'Ouvindo…':Platform.OS==='web'?'Falar agora':'Voz via Staff'}</Text></Pressable><TextInput value={command} onChangeText={(value)=>{setCommand(value);setPreview(null);}} multiline placeholder="Ex.: Conta a pagar: energia | 480 | 2026-08-25" placeholderTextColor={colors.textMuted} style={styles.input}/><Button label="Interpretar comando" disabled={!command.trim()} onPress={analyze}/></View>
...
function parseCommand(raw:string,members:any[]):Preview{const text=raw.trim();if(!text)throw new Error('Diga ou digite um comando.');const normalized=text.toLowerCase();if(normalized.includes('o que tenho hoje')||normalized==='hoje'||normalized.includes('tarefas de hoje'))return{kind:'TODAY'};const obligation=text.match(/(?:obrigação|obrigacao)\s*:\s*(.+?)\s*\|\s*(\d{4}-\d{2}-\d{2})/i);if(obligation)return{kind:'OBLIGATION',title:obligation[1].trim(),dueDate:obligation[2]};const payable=text.match(/conta a pagar\s*:\s*(.+?)\s*\|\s*([\d.,]+)\s*\|\s*(\d{4}-\d{2}-\d{2})/i);if(payable)return{kind:'PAYABLE',description:payable[1].trim(),amount:money(payable[2]),dueDate:payable[3]};const charge=text.match(/(?:cobrança|cobranca)\s*:\s*(.+?)\s*\|\s*([\d.,]+)\s*\|\s*(\d{4}-\d{2}-\d{2})(?:\s*\|\s*(.+))?/i);if(charge){const query=charge[1].trim().toLowerCase();const member=members.find((m:any)=>m.name.toLowerCase().includes(query)||m.email?.toLowerCase()===query);if(!member)throw new Error(`Não encontrei “${charge[1].trim()}” entre os membros da Loja.`);return{kind:'CHARGE',memberId:member.id,memberName:member.name,memberEmail:member.email,memberPhone:member.whatsapp,amount:money(charge[2]),dueDate:charge[3],description:charge[4]?.trim()||'Mensalidade'};}const publish=text.match(/(?:comunicado|publique|publicar)\s*:\s*(.+?)(?:\s*\|\s*(.+))?$/i);if(publish)return{kind:'PUBLISH',title:publish[1].trim(),summary:publish[2]?.trim()||publish[1].trim()};throw new Error('Ainda não reconheci esse formato. Use um dos exemplos abaixo; vamos ampliar o vocabulário com o motor Staff.');}
