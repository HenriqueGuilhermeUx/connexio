...
import { FinanceDocument, getFinanceDocumentUrl, pickFinanceDocument, uploadFinanceDocument } from '@/lib/financeDocuments';
...
export default function ManagerFinanceScreen() {
  const { lodge, financialEntries, createFinancialEntry, markFinancialEntryPaid } = useApp();
  ...
  const chooseDocument = async () => {
    try {
      const picked = await pickFinanceDocument();
      if (picked) setDocument(picked);
    } catch (error) {
      Alert.alert('Não foi possível selecionar o arquivo', error instanceof Error ? error.message : 'Tente novamente.');
    }
  };
  ...
  const add = async () => {
    ...
    try {
      ...
      if (document && lodge) {
        const upload = await uploadFinanceDocument(lodge.id, document);
        attachmentName = upload.path;
      } else if (document) {
      ...
      setDescription(''); setCategory(''); setAmount(''); setDueDate(''); setRecurring(false); setDocument(null);
  ...
  const openAttachment = async (path: string) => {
    try {
      const url = await getFinanceDocumentUrl(path);
      if (url) await Linking.openURL(url);
      else Alert.alert('Documento', 'O arquivo está disponível apenas quando o backend estiver conectado.');
    } catch (error) {
