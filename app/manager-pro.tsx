...
export default function ManagerProScreen() {
  ...
  return <Screen contentStyle={styles.content}>
    ...
    <View style={styles.priceCard}><View><Text style={styles.plan}>Gestor Pro</Text><Text style={styles.price}>R$ 49,90<Text style={styles.priceSuffix}> / mês por Loja</Text></Text></View><View style={styles.status}><Text style={styles.statusText}>{lodge?.plan === 'PRO' ? 'PRO ATIVO' : 'PRÉVIA'}</Text></View></View>
    <View style={styles.grid}>{modules.map((module)=><Pressable key={module.title} disabled={!canManage} onPress={()=>router.push(module.route)} style={({pressed})=>[styles.moduleCard,!canManage&&styles.disabled,pressed&&styles.pressed]}><View style={styles.moduleIcon}><Feather name={module.icon} size={21} color={colors.gold}/></View><Text style={styles.moduleTitle}>{module.title}</Text><Text style={styles.moduleDescription}>{module.description}</Text><Text style={styles.open}>Abrir →</Text></Pressable>)}</View>
