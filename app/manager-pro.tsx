...
export default function ManagerProScreen(){
  ...
  useEffect(()=>{if(lodge&&isSupabaseConfigured)void loadLatestGestorProRequest(lodge.id).then(setRequest).catch(()=>undefined);},[lodge?.id,lodge?.plan]);
  const askPro=async()=>{if(!lodge)return;try{const row=await requestGestorPro(lodge.id);setRequest(row);Alert.alert('Solicitação enviada','A ativação entrou na fila comercial do Connexio. O plano custa R$ 49,90/mês por Loja.');}catch(error){Alert.alert('Não foi possível solicitar',error instanceof Error?error.message:'Tente novamente.');}};
